...
data$quantization_type <- factor(data$quantization_type, levels = c("32-bit", "8-bit", "4-bit"))
...
delta_32_8 <- cliff.delta(data$GPU.Energy[data$quantization_type == "32-bit"],
                          data$GPU.Energy[data$quantization_type == "8-bit"])

# Calculate Cliff's Delta for 32-bit vs 4-bit
delta_32_4 <- cliff.delta(data$GPU.Energy[data$quantization_type == "32-bit"],
                          data$GPU.Energy[data$quantization_type == "4-bit"])

# Calculate Cliff's Delta for 8-bit vs 4-bit
delta_8_4 <- cliff.delta(data$GPU.Energy[data$quantization_type == "8-bit"],
                         data$GPU.Energy[data$quantization_type == "4-bit"])
