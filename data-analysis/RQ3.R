...
#data <- data %>%
#  mutate(GPU.Util = GPU.Busy.Time / Inference.Time)

#data <- data %>%
#  mutate(CPU.Util = CPU.Busy.Time / Inference.Time)

summary(data)

# bar plot
ggplot(data, aes(x = quantization_type)) +
  geom_bar(aes(y = CPU.Busy.Time, fill = "CPU Busy Time"), stat = "identity", width = width, position = position_nudge(x = -0.15)) + 
  geom_bar(aes(y = GPU.Busy.Time, fill = "GPU Busy Time"), stat = "identity", width = width, position = position_nudge(x = 0.15)) +
